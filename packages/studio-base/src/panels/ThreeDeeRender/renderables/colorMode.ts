...
function bluegoldLookup(output: ColorRGBA, pct: number): void {
  ...
  if (blueGoldColormapData[pct] != undefined) {
    output.r = SRGBToLinear(blueGoldColormapData[pct]![0]! / 255.0);
    output.g = SRGBToLinear(blueGoldColormapData[pct]![1]! / 255.0);
    output.b = SRGBToLinear(blueGoldColormapData[pct]![2]! / 255.0);
    output.a = 1;
  }
}
